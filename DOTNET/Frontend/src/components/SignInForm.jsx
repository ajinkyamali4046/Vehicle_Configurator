...
function SignInForm() {
  ...
  //     const loginResponse = await fetch("http://localhost:5254/api/User/login", {
  //       method: "POST",
  //       headers: {
  ...
  const handleOnSubmit = async (evt) => {
    ...
    try {
      ...
      // If login is successful, extract the response data
      const loginData = await loginResponse.json();
      if (loginData) {
        // Store necessary data (e.g., user ID) in sessionStorage or localStorage
        localStorage.setItem("username", state.username);
        console.log("Username stored:", state.username);

        // Call login function from context (if necessary)
        login(); // Pass relevant data if needed
