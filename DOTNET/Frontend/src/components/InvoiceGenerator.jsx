...
function InvoiceGenerator() {
  ...
  console.log(selectedDropdownIds);
  ...
  useEffect(() => {
    if (selectedId) {
      // Fetch model price
      fetch(`http://localhost:5254/api/Model/${selectedId}`, {
        method: "GET",
        // headers: {
        ...
        .then((data) => {
          console.log("delllllllll"+(location.state?.totalDeltaa));
    ...
    if (selectedDropdownIds.length > 0) {
      ...
      Promise.all(
        selectedDropdownIds.map((id) =>
          fetch(`http://localhost:5254/api/Component/${id}`, {
            method: "GET",
            // headers: { 
            //   Authorization: `Bearer ${token}`, // Include token in headers
            //   "Content-Type": "application/json"
  ...
  useEffect(() => {
    if (selectedId) {
      ...
      fetch(`http://localhost:5254/api/Vehicle/config/${selectedId}/n`, {
        method: "GET",
        // headers: {
        //   Authorization: `Bearer ${token}`, // Include token in headers
        //   "Content-Type": "application/json"
...
const generateAndDownloadPDF = () => {
  ...
  img.onload = function() {
    ...
    // Adding non-configurable components
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.text('Non-Configurable Components:', 10, currentY);
  
    currentY += verticalSpace;
  
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(12);
    nonConfigurableComponents.forEach((name, index) => {
  ...
  const handleConfirmOrder = async () => {
    ...
    //   }
      ...
      const userResponse = await fetch(`http://localhost:5254/api/User/${UN}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`, // Include token in headers
          "Content-Type": "application/json"
      ...
      const emailResponse = await fetch(`http://localhost:5254/api/Email/sendMailWithAttachment`, {
        method: "POST",
        mode: "cors",
        headers: {
          // Authorization: `Bearer ${token}`, // Include token in headers
  ...
  return (
    <div>
      ...
      <h2 style={{ marginLeft: "10px" }}>Non-Configurable components:</h2>
